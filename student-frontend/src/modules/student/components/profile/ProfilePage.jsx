  ...
  const fetchEmployeeDetails = useCallback(async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/api/v1/student/campus/${campusId}/me`,
        {
          headers: {
    ...
    <div className="w-full ml-6 rounded-xl p-4">
      ...
      <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 mt-6">
        ...
        <div>
          ...
          <div><span className="font-semibold">Student ID</span>: {studentId}</div>
            <div><span className="font-semibold">Admission No</span>: {details?.admissionNumber}</div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mt-6">
        <div className="flex-1 bg-gray-200 rounded-2xl py-4 text-center">
          ...
          <div className="text-sm">Total Class Days</div>
        </div>
        <div className="flex-1 bg-gray-200 rounded-2xl py-4 text-center">
