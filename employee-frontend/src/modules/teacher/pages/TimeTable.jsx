...
const TimeTable = () => {
  ...
  async function fetchTimeTable() {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/api/v1/timeTable/fetch-byDepartment/${data.departmentId}`
      );
      // console.log(response.data.timeTable[0]);
      if (response.data.timeTable.length === 0) {
        const postResponse = await axios.post(`${import.meta.env.VITE_BASE_URL}/api/v1/timeTable/upload/${data.id}`,{
  ...
  async function fetchDept() {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/api/v1/department/fetch/${data.departmentId}`
      );
  ...
  async function fetchAllTeachers() {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/api/v1/teacher/${data.campusId}/fetchAll`
      );
  ...
  const fetchHolidays = useCallback(async () => {
	...
	try {
		// Ensure campusId is added to the newHoliday object
		const holidayData = {
			...newHoliday,
			campusId: data.campusId,
		};

		// Send the POST request with the updated holidayData
		const response = await axios.post(
			`${import.meta.env.VITE_BASE_URL}/api/v1/holiday/create/${data.id}`,
			holidayData
		);
	...
	const updateHoliday = async () => {
		...
		try {
			const response = await axios.put(
				`${import.meta.env.VITE_BASE_URL}/api/v1/holiday/update/${data.id}/${editingHoliday}`,
				editHolidayData,
	...
	const deleteHoliday = async (holidayId) => {
		try {
			await axios.delete(
				`${import.meta.env.VITE_BASE_URL}/api/v1/holiday/delete/${data.id}/${holidayId}`,
				{ headers: { Authorization: `Bearer ${data.token}` } }
			);
			setHolidays((prevHolidays) => {
				const updatedHolidays = { ...prevHolidays };
  ...
  async function saveTimeTable() {
    ...
    try {
      const response = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/v1/timeTable/update/${data.id}`,
        {
          ...timeTable,
          departmentId: data.departmentId,
        }
